...
- **`scripts/fetch_transcript.py`** — YouTubeから字幕を取得してJP/ENペアを推定したJSONを出力するPythonスクリプト（レガシー）。
...
## YouTube APIキーの取得（レガシー機能）

`fetch_transcript.py` の `--channel` モードで使う場合のみ必要。
